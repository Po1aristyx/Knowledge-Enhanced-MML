...
import sys
import torch
log_file_check = "log_NACC_ConvR.txt"
if os.path.exists(log_file_check):
    with open(log_file_check, 'r', encoding='utf-8', errors='ignore') as f:
        if "=== Final Validation Metrics ===" in f.read():
            print(f"[NACC ConvR] Already completed. Skipping.")
            sys.exit(0)
            
os.makedirs("checkpoints", exist_ok=True)
ckpt_path = f"checkpoints/ckpt_kamt_NACC_ConvR.pth"
...
# 保持你的路径配置不变
ENTITY_EMB_NPY = "NACC-ConvR-Embeddings.npy"
ENTITY2ID_FILE = "NACC-ConvR-Entity2ID.json"
