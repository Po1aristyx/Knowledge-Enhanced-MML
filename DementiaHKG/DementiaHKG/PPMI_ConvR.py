...
import sys
import torch
log_file_check = "log_PPMI_ConvR.txt"
if os.path.exists(log_file_check):
    with open(log_file_check, 'r', encoding='utf-8', errors='ignore') as f:
        if "=== Final Validation Metrics ===" in f.read():
            print(f"[PPMI ConvR] Already completed. Skipping.")
            sys.exit(0)
            
os.makedirs("checkpoints", exist_ok=True)
ckpt_path = f"checkpoints/ckpt_kamt_PPMI_ConvR.pth"
...
# 读取PPMI数据集对应的图谱特征文件
ENTITY_EMB_NPY = "PPMI-ConvR-Embeddings.npy"
ENTITY2ID_FILE = "PPMI-ConvR-Entity2ID.json"
