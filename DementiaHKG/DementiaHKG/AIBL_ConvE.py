...
import sys
import torch
log_file_check = "log_AIBL_ConvE.txt"
if os.path.exists(log_file_check):
    with open(log_file_check, 'r', encoding='utf-8', errors='ignore') as f:
        if "=== Final Validation Metrics ===" in f.read():
            print(f"[AIBL ConvE] Already completed. Skipping.")
            sys.exit(0)
            
os.makedirs("checkpoints", exist_ok=True)
ckpt_path = f"checkpoints/ckpt_kamt_AIBL_ConvE.pth"
...
ENTITY_EMB_NPY = "AIBL-ConvE-Embeddings.npy"
ENTITY2ID_FILE = "AIBL-ConvE-Entity2ID.json"
