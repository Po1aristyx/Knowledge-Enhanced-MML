...
import sys
import torch
log_file_check = "log_PPMI_ConvE.txt"
if os.path.exists(log_file_check):
    with open(log_file_check, 'r', encoding='utf-8', errors='ignore') as f:
        if "=== Final Validation Metrics ===" in f.read():
            print(f"[PPMI ConvE] Already completed. Skipping.")
            sys.exit(0)
            
os.makedirs("checkpoints", exist_ok=True)
ckpt_path = f"checkpoints/ckpt_kamt_PPMI_ConvE.pth"
...
count_swedd = 0
count_prodromal = 0
with open(r'd:/Polaris/Documents/work4/Baseline_CustomKG/Baseline&CustomKG/Baseline/PPMI/PD1.csv', mode='r', newline='', encoding='utf-8') as file:
    csv_reader = csv.reader(file)
    next(csv_reader)  
    for row in csv_reader:
        path = 'E:/code/dazuoyecode/PPMI/PD/' + row[1]
...
with open(r'd:/Polaris/Documents/work4/Baseline_CustomKG/Baseline&CustomKG/Baseline/PPMI/control.csv', mode='r', newline='', encoding='utf-8') as file:
    csv_reader = csv.reader(file)
    next(csv_reader) 
    for row in csv_reader:
        path = 'E:/code/dazuoyecode/PPMI/Control/' + row[1]
        exists = os.path.exists(path)
        path_existence.append((path, exists))
        if exists:
...
with open(r'd:/Polaris/Documents/work4/Baseline_CustomKG/Baseline&CustomKG/Baseline/PPMI/swedd.csv', mode='r', newline='', encoding='utf-8') as file:
    csv_reader = csv.reader(file)
    next(csv_reader) 
    for row in csv_reader:
        path = 'E:/code/dazuoyecode/PPMI/SWEDD/' + row[1]
        exists = os.path.exists(path)
        path_existence.append((path, exists))
        if exists:
...
with open(r'd:/Polaris/Documents/work4/Baseline_CustomKG/Baseline&CustomKG/Baseline/PPMI/prodromal.csv', mode='r', newline='', encoding='utf-8') as file:
    csv_reader = csv.reader(file)
    next(csv_reader) 
    for row in csv_reader:
        path = 'E:/code/dazuoyecode/PPMI/Prodromal/' + row[1]
        exists = os.path.exists(path)
        path_existence.append((path, exists))
        if exists:
...
# 1. 严格用csv顺序生成影像路径
pd_img_paths = [os.path.join('E:/code/dazuoyecode/PPMI/PD', row[1]) for row in data_PD if os.path.exists(os.path.join('E:/code/dazuoyecode/PPMI/PD', row[1]))]
control_img_paths = [os.path.join('E:/code/dazuoyecode/PPMI/Control', row[1]) for row in data_control if os.path.exists(os.path.join('E:/code/dazuoyecode/PPMI/Control', row[1]))]
swedd_img_paths = [os.path.join('E:/code/dazuoyecode/PPMI/SWEDD', row[1]) for row in data_swedd if os.path.exists(os.path.join('E:/code/dazuoyecode/PPMI/SWEDD', row[1]))]
prodromal_img_paths = [os.path.join('E:/code/dazuoyecode/PPMI/Prodromal', row[1]) for row in data_prodromal if os.path.exists(os.path.join('E:/code/dazuoyecode/PPMI/Prodromal', row[1]))]
...
# 读取PPMI数据集对应的图谱特征文件
ENTITY_EMB_NPY = "PPMI-ConvE-Embeddings.npy"
ENTITY2ID_FILE = "PPMI-ConvE-Entity2ID.json"
...
def get_patient_embedding_seq(subject_id):
    ...
    pad_t = torch.tensor(pad_masks, dtype=torch.bool)
...
pd_df        = pd.read_csv(r'd:/Polaris/Documents/work4/Baseline_CustomKG/Baseline&CustomKG/Baseline/PPMI/PD1.csv')
control_df = pd.read_csv(r'd:/Polaris/Documents/work4/Baseline_CustomKG/Baseline&CustomKG/Baseline/PPMI/control.csv')
prodromal_df = pd.read_csv(r'd:/Polaris/Documents/work4/Baseline_CustomKG/Baseline&CustomKG/Baseline/PPMI/prodromal.csv')
swedd_df = pd.read_csv(r'd:/Polaris/Documents/work4/Baseline_CustomKG/Baseline&CustomKG/Baseline/PPMI/swedd.csv')

print("开始提取PPMI知识序列与注意力掩码...")
pd_seq, pd_mask               = process_and_verify_seq(pd_df, "E:/code/dazuoyecode/PPMI/PD")
control_seq, control_mask     = process_and_verify_seq(control_df, "E:/code/dazuoyecode/PPMI/Control")
prodromal_seq, prodromal_mask = process_and_verify_seq(prodromal_df, "Prodromal")
swedd_seq, swedd_mask         = process_and_verify_seq(swedd_df, "E:/code/dazuoyecode/PPMI/SWEDD")
...
plt.legend()
plt.grid(True)
plt.savefig('PPMI_DementiaHKG_plot.png', dpi=300)
plt.close()
