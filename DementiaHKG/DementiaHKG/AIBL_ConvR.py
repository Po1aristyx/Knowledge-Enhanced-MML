...
import sys
import torch
log_file_check = "log_AIBL_ConvR.txt"
if os.path.exists(log_file_check):
    with open(log_file_check, 'r', encoding='utf-8', errors='ignore') as f:
        if "=== Final Validation Metrics ===" in f.read():
            print(f"[AIBL ConvR] Already completed. Skipping.")
            sys.exit(0)
            
os.makedirs("checkpoints", exist_ok=True)
ckpt_path = f"checkpoints/ckpt_kamt_AIBL_ConvR.pth"
...
with open(r'd:/Polaris/Documents/work4/Baseline_CustomKG/Baseline&CustomKG/Baseline/AIBL/NC.csv', mode='r', newline='', encoding='utf-8') as file:
    ...
    for row in csv_reader:
        path = 'E:/code/dazuoyecode/AIBL/NC/' + row[1]
...
with open(r'd:/Polaris/Documents/work4/Baseline_CustomKG/Baseline&CustomKG/Baseline/AIBL/AD.csv', mode='r', newline='', encoding='utf-8') as file:
    ...
    next(csv_reader) 
    for row in csv_reader:
        path = 'E:/code/dazuoyecode/AIBL/AD/' + row[1]
        exists = os.path.exists(path)
        path_existence.append((path, exists))
        if exists:
            count_ad=count_ad+1
...
with open(r'd:/Polaris/Documents/work4/Baseline_CustomKG/Baseline&CustomKG/Baseline/AIBL/MCI.csv', mode='r', newline='', encoding='utf-8') as file:
    ...
    next(csv_reader) 
    for row in csv_reader:
        path = 'E:/code/dazuoyecode/AIBL/MCI/' + row[1]
        exists = os.path.exists(path)
        path_existence.append((path, exists))
        if exists:
...
# 3. 模块化调用，保留原始文件夹名称
ad_output = extract_mri_features('E:/code/dazuoyecode/AIBL/AD')
print('ad nii shape--->', ad_output.shape)

normal_output = extract_mri_features('E:/code/dazuoyecode/AIBL/NC')
print('normal nii shape--->', normal_output.shape)

mci_output = extract_mri_features('E:/code/dazuoyecode/AIBL/MCI')
...
ENTITY_EMB_NPY = "AIBL-ConvR-Embeddings.npy"
ENTITY2ID_FILE = "AIBL-ConvR-Entity2ID.json"
