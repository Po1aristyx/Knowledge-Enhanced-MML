...
def center_crop_3d(tensor, size):
    ...
    assert depth >= target_depth and height >= target_height and width >= target_width, \
        f"输入尺寸{tensor.shape}小于裁剪尺寸{size}"
...
class NiiDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_tensor = cropped_tensor.unsqueeze(0)  # (1, 64, 64, 64)
        assert input_tensor.shape == (1, 64, 64, 64), f"实际: {input_tensor.shape}"
        return input_tensor
